...
import static com.raelity.astrolog.castro.Castro.getCompactAstrologVersionString;
...
{
    // only want the "AS760" to show up in the .as file which uses ';'
    String commentAS = ";".equals(commentLeader) ? "" : "//";

    if(lookup(CastroOutputOptions.class).outputOpts().contains(GENERAL_ANON)) {
        out.printf(" %sAS\n%s Compiled for Astrolog\n",
                   commentAS, commentLeader);
    ...
    out.printf(" %sAS%s\n%s Compiled against Astrolog v%s\n",
               commentAS,
               getCompactAstrologVersionString(), commentLeader,
               Castro.getAstrologVersionString());
    out.printf("%s Generated by castro v%s on %s\n",
               commentLeader, Castro.getVersionString(), runDate);
}
