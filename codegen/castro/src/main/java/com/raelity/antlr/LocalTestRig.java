...
public class LocalTestRig {
	...
	public LocalTestRig(String[] args) throws Exception {
		if ( args.length < 2 ) {
			System.err.println("""
		...
		while ( i<args.length ) {
			...
			if ( arg.equals("-tokens") ) {
				...
				if ( i>=args.length ) {
					System.err.println("missing listener class name on  -listener");
					return;
				}
				listenerClassName = args[i];
				i++;
			}
		}
	}
	...
	public static void main(String[] args) throws Exception {
		LocalTestRig testRig = new LocalTestRig(args);
 		if(args.length >= 2) {
			testRig.process();
		}
	}
